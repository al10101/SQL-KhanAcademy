...
SELECT gender, AVG(strength) AS avg_strength FROM marvels GROUP BY gender;

SELECT name, popularity FROM marvels GROUP BY popularity HAVING popularity <= 10;

SELECT name, weight_kg, height_m, 
    CASE
        WHEN imc > 40 THEN "Grade III Obesity"
        WHEN imc > 35 THEN "Grade II Obesity"
        WHEN imc > 30 THEN "Grade I Obesity"
        WHEN imc > 25 THEN "Overweight"
        WHEN imc > 18.5 THEN "Normal"
        ELSE "Too skinny to classify"
    END AS "imc_classification"
FROM (
    SELECT *, (weight_kg / (height_m*height_m)) as imc FROM marvels
) ORDER BY imc_classification;

/* A lot of those characters are way to heavy, maybe their weight is in pounds */

SELECT name, real_weight_kg, height_m, 
    CASE
        WHEN imc > 40 THEN "Grade III Obesity"
        WHEN imc > 35 THEN "Grade II Obesity"
        WHEN imc > 30 THEN "Grade I Obesity"
    ...
    END AS "imc_classification"
FROM (
    SELECT *, (real_weight_kg / (height_m*height_m)) as imc FROM ( SELECT *, 
            CASE 
                WHEN weight_kg > 150 AND height_m < 2.50 THEN weight_kg / 2
                ELSE weight_kg
            END AS real_weight_kg
        FROM marvels
    )
) WHERE imc_classification LIKE "%Grade III%" ORDER BY imc_classification;
